...
async def on_anime_select(client: Client, callback_query):
    ...
    # Handle both anilist_ID format and plain anime_id
    raw = callback_query.data[len("anime_"):]  # everything after "anime_"
    is_anilist = raw.startswith("anilist_")
    anime_id = raw[len("anilist_"):] if is_anilist else raw

    await callback_query.answer("⏳ ꜰᴇᴛᴄʜɪɴɢ ᴇᴘɪꜱᴏᴅᴇꜱ...")

    entries = []
    url = ""
    anime_title = "Unknown"

    # If AniList result — get episodes via animetsu using anilist_id mapping
    if is_anilist:
        # Try animetsu first (it uses AniList IDs natively)
        try:
            from cantarella.scraper.animetsu import AnimetsuScraper
            scraper = AnimetsuScraper()
            # Search by AniList ID
            info = await client.loop.run_in_executor(None, scraper.get_anime_info, anime_id)
            if info:
                anime_title = info.get("title", "Unknown")
                entries = await client.loop.run_in_executor(None, scraper.list_episodes, anime_id)
                url = f"https://animetsu.live/anime/{anime_id}"
        except Exception as e:
            print(f"Animetsu fetch failed for anilist_{anime_id}: {e}")

        # If no entries from animetsu, tell user to provide direct URL
        if not entries:
            user_results = user_search_results.get(callback_query.from_user.id, [])
            selected = next((r for r in user_results if str(r.get("anilist_id")) == str(anime_id)), None)
            title = selected.get("title", "this anime") if selected else "this anime"
            cover = selected.get("cover", "") if selected else ""

            status_txt = (
                f"<blockquote>📺 <b>{title}</b>\n\n"
                f"⚠️ ᴇᴘɪꜱᴏᴅᴇ ꜱᴛʀᴇᴀᴍɪɴɢ ɪꜱ ɴᴏᴛ ᴀᴠᴀɪʟᴀʙʟᴇ ʀɪɢʜᴛ ɴᴏᴡ.\n\n"
                f"Please paste a direct episode URL from:\n"
                f"• animetsu.live\n"
                f"• aniwatchtv.to\n"
                f"• hianime.to</blockquote>"
            )
            try:
                if cover:
                    ...
                    await callback_query.message.edit_caption(status_txt, parse_mode=ParseMode.HTML)
            except Exception:
                await callback_query.message.reply(status_txt, parse_mode=ParseMode.HTML)
            return
    else:
        # Regular scraper ID
    ...
    if not entries:
        await callback_query.answer("❌ ɴᴏ ᴇᴘɪꜱᴏᴅᴇꜱ ꜰᴏᴜɴᴅ.", show_alert=True)
        return

    user_episodes[callback_query.from_user.id] = {
        "title":    entries[0].get("title", anime_title),
        "episodes": entries,
        "url":      url,
        "page":     0
    }
